...
#include "ShadowLevelTest.h"

#include "Core/Math/Matrix.h"
#include "Core/Math/Quaternion.h"


void ShadowLevelTest::InitLevel()
{
...
#pragma region Gen plane VAO
	...
	glGenVertexArrays(1, &VAOPlaneTest_);
	glBindVertexArray(VAOPlaneTest_);

	{
		const float halfLength = 50.0f, height = 10.0f;

		GLfloat quadVertexData[] = {
			-halfLength, height, -halfLength,
			halfLength, height, -halfLength,
			halfLength, height, halfLength,
			-halfLength, height, halfLength,
		};
...
#pragma region Compile shader and get location
	...
	diffuseTexture = Texture2D::LoadFromPath(teapot_.M(0).map_Kd, 4.0f);
	specularTexture = Texture2D::LoadFromPath(teapot_.M(0).map_Ks, 4.0f);

	depthFrameBuffer_ = DepthBuffer::MakeDepthBuffer();
	lightDiffuseColor_ = Vector3(0.58823532f, 0.58823532f, 0.58823532f);
#pragma endregion
...
void ShadowLevelTest::DisplayLevel()
{
	...
	depthFrameBuffer_->BindBufferTexture(depthTexture_);
	if (!depthFrameBuffer_->CheckStatus())
		return;
	...
	glUniformMatrix4fv(lightSpaceLocation_, 1, GL_TRUE, &lightSpaceMatrix.M[0][0]);
	...
	glUniformMatrix4fv(planeFTMLocation_, 1, GL_TRUE, &lightSpaceMatrix.M[0][0]);
	glUniformMatrix4fv(planeLightSpaceMatrixLocation_, 1, GL_TRUE, &lightSpaceMatrix.M[0][0]);

	depthTexture_->Bind(GL_TEXTURE0);
	glUniform1i(depthTextureLocation_, 0);
...
void ShadowLevelTest::CloseLevel()
{
